...
footerFirstLine.innerHTML = footerFirstLine.innerHTML.replaceAll("&nbsp;", " ");
const formattedFooterFirstLine = footerFirstLine.innerText.replace('ПРЕДСЕДАТЕЛЬ ПАРЛАМЕНТА', 'Председатель парламента')
const footerSecondLine = allParagraphs[allParagraphs.length - 1];
footerSecondLine.innerHTML = footerSecondLine.innerHTML.replaceAll("&nbsp;", " ");
...
// change p with sup to h3
document.querySelectorAll('main > p > strong > span > span > span > sup').forEach(p => {
  const firstSpan = p.parentElement;
  const secondSpan = p.parentElement.parentElement.parentElement.parentElement.nextElementSibling;
  secondSpan.innerHTML = secondSpan.innerHTML.replace("&nbsp;", "");
  const parent = secondSpan.parentElement;
  parent.outerHTML = `<h3>${firstSpan.innerHTML} ${secondSpan.innerText}</h3>`;
});

// change p to h3
document.querySelectorAll('main > p > strong + span + em').forEach(em => {
...
document.querySelectorAll('main > p > span > span > span > strong').forEach(strong => {
...
// format expired article points
document.querySelectorAll('main > p > span  + em').forEach(em => {
  const emChild = em.firstChild.firstChild.firstChild;
  emChild.innerHTML = emChild.innerHTML.replaceAll("&nbsp;", " ");
